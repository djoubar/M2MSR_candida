...
df_base <- read_xlsx("~/M2MSR/donnees/data_chu.xlsx") |>
  set_names(tolower) |>
...
df_base <- df_base |>
  mutate(
    ...
    adm_cgr = as.factor(adm_cgr),
    adm_pfc = as.factor(adm_pfc),
    adm_cp = as.factor(adm_cp),
    # hc_vi_cat = factor(hc_vi_cat, levels = c("non", "oui"), labels = c("Non", "Oui")),
    hc_cgr = as.factor(hc_cgr),
    hc_pfc = as.factor(hc_pfc),
    hc_cp = as.factor(hc_cp),
...
df_base <- df_base |>
  mutate(
    adm_choc = as.factor(ifelse(
      adm_noradre == "TRUE" | adm_sofa_cardio != 0 | adm_lactates_max > 4,
      1,
      0
    )),
    hc_choc = as.factor(ifelse(
      hc_noradre == "TRUE" | hc_sofa_cardio == "4" | hc_lactates_max > 4 | hc_sofa_cardio == "3",
      1,
      0
    )),
    ...
    hc_glucanes_max = ifelse(
      is.na(hc_glucanes_max),
      0,
      hc_glucanes_max
    ),
    hc_mannanes_max = ifelse(
      is.na(hc_mannanes_max),
      0,
      hc_mannanes_max
    ),
...
# .df_base.imp <- df_base[,c(10:112)]
# .df_base.mice <- mice(.df_base.imp, seed = 1)
# df_base <- complete(.df_base.mice, 1)
